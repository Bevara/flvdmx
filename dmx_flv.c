...
typedef struct {
    ...
    u8 sps[1024], pps[256], vps[64];
    u32 sps_len, pps_len, vps_len;
    u32 sps_num, pps_num, vps_num;
...
static void flvdmx_video_callback(enum FLVVideoType video_type, int64_t timestamp, uint8_t *data, uint32_t data_len, void *arg)
{
    ...
    if (!ctx->v_opid || ctx->video_done) return;
    
    GF_FilterPacket *pck = gf_filter_pck_new_alloc(ctx->v_opid, data_len, NULL);
    if (!pck) return;
    
    u8 *out;
    gf_filter_pck_get_data_mut(pck, &out);
    memcpy(out, data, data_len);
    
    gf_filter_pck_set_cts(pck, (double)timestamp / 1000.0);
    gf_filter_pck_set_duration(pck, 0.04);
...
static void flvdmx_audio_callback(enum FLVAudioType audio_type, int profile, int sample_rate_index, int channel, int64_t timestamp, uint8_t *data, uint32_t data_len, void *arg)
{
    ...
    GF_FilterPacket *pck = gf_filter_pck_new_alloc(ctx->a_opid, data_len, NULL);
    if (!pck) return;
...
static void flvdmx_configure_pid(GF_Filter *filter, GF_FilterPid *pid, Bool configured)
{
...
static GF_Err flvdmx_process_event(GF_Filter *filter, GF_FilterEvent evt, GF_FilterEventReq *req)
{
    GF_FLVDMXCtx *ctx = gf_filter_get_udta(filter);
    
    if (evt == GF_FEVT_PLAY_HINT && req) {
        if (req->play_hint.full_file_only == GF_TRUE) {
            return GF_OK;
        }
    }
